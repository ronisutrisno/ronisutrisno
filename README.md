...
| Fitur | Keterangan |
|-------|------------|
| **Chat streaming** | Respons real-time token per token; multi-model via Admin. |
| **RAG** | Unggah dokumen/gambar/audio sebagai konteks; drag-drop atau paste URL (fetch konten otomatis). |
...
| Feature | Description |
|---------|-------------|
| **Chat streaming** | Real-time token-by-token response; multi-model via Admin. |
| **RAG** | Upload documents/images/audio as context; drag-drop or paste URL (auto-fetch content). |
